...
export const recoverPassAction = async (req, res) => {
  ...
  if (!isValidPassword(result, password)) {
    ...
    if (newUserPassword) res.render('session-views/login', { message: 'se actualizo su contraseña' })
  } else {
    res.render('session-views/login', { error: 'LA CONTRASEÑA NO PUEDE SER IGUAL A LA ANTERIOR' })
  }
}
